...
const requireCustomerData=(req: Request, res: Response, next: NextFunction)=>process.env.String(process.env.CUSTOMER_DATA_ENABLED).toLowerCase()==='true'?next():res.status(404).json({error:'Customer data is disabled'});
const requireGptAction=(req: Request, res: Response, next: NextFunction)=>{if(process.env.GPT_ACTIONS_ENABLED!=='true')return res.status(404).json({error:'GPT Actions are disabled'});const supplied=req.header('authorization')?.replace(/^Bearer\s+/i,'')||'';const expected=process.env.GPT_ACTIONS_API_KEY||'';if(!expected||supplied.length!==expected.length||!crypto.timingSafeEqual(Buffer.from(supplied),Buffer.from(expected)))return res.status(401).json({error:'Unauthorized'});next()};

app.get('/api/health',(_req: Request, res: Response)=>res.json({ok:true,mode:'sandbox',liveProcessing:false,providers:{stripeTestConfigured:Boolean(process.env.STRIPE_SECRET_KEY?.startsWith('sk_test_')),stripeCheckoutEnabled:process.env.STRIPE_CHECKOUT_ENABLED==='true',stripeWebhookConfigured:Boolean(process.env.STRIPE_WEBHOOK_SECRET?.startsWith('whsec_')),paypal:process.env.PAYPAL_ENABLED==='true',crypto:process.env.CRYPTO_PAYMENTS_ENABLED==='true'}}));
...
app.get('/api/payment-links',requireRole('admin','finance','support','viewer'),(_req: Request, res: Response)=>res.json({data:paymentLinks}));
